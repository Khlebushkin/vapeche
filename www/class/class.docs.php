...
//			if($row->document_lang_group > 0){
				$sql1=$AVE_DB->Query("
					...
					FROM
						".PREFIX."_documents
					WHERE
						document_lang_group=".$row->document_lang_group." OR Id=".$row->document_lang_group);
...
//			}
	...
	*/
	...
	function documentSave($rubric_id, $document_id, $data, $update_non_exists_fields = false, $rubric_code = true, $revisions = true, $logs = true)
	{
		...
		if($oper == 'UPDATE')
		{
				...
				else
				{
					...
					$data['document_sitemap_freq']	= $data['document_sitemap_freq'] ? $data['document_sitemap_freq'] : '3';
					$data['document_sitemap_pr']	= $data['document_sitemap_pr'] ? $data['document_sitemap_pr'] : '0.5';
