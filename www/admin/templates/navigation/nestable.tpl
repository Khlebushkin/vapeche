	...
	{foreach from=$items key=key item=item}
		<li class="dd-item dd3-item" data-id="{$item.navigation_item_id}" id="item-{$item.navigation_item_id}">
			...
			<div class="dd3-content{if $item.status == 0} red{/if}">
				...
				<div class="action">
					<a href="index.php?do=navigation&action=itemedit&sub=edit&navigation_item_id={$item.navigation_item_id}&cp={$sess}&pop=1" data-width="420px" data-height="620" data-modal="true" data-dialog="item-{$item.navigation_item_id}" data-title="{#NAVI_ITEM_EDIT#}" title="{#NAVI_ITEM_EDIT#}" class="openDialog topleftDir icon_sprite ico_edit"></a>
