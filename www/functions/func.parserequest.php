		...
		if(!file_exists($cachefile_docid))
			{
				...
				*/
				...
				$item = str_replace('[tag:adminlink]', 'index.php?do=docs&action=edit&rubric_id='.$row->rubric_id.'&Id='.$row->Id.'&cp='. SESSION .'', $item);
...
function request_parse($id, $params=array())
{
	...
	// если просили просто вернуть запрос
	if ($params['RETURN_SQL'] == 1)
		return $sql;

	if ($request->request_show_pagination == 1 || $params['SHOW'] == 1)
	{
